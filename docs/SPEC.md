...
- **Hilfsprozesse unsichtbar:** Unter Windows öffnet weder ein ExifTool-Prozess noch der Arbeitsprozess der Oberfläche ein Konsolenfenster (`CREATE_NO_WINDOW`, Startinformation `SW_HIDE` — `prozesse.py`, gilt auch im gepackten Programm). Im ersten echten Testlauf ging je ExifTool-Prozess ein schwarzes Fenster auf, und der Virenscanner schlug an. Die Paketprüfung in der CI beobachtet während des Durchlaufs alle sichtbaren Fenster: Außer dem Hauptfenster darf keines aufgehen.
