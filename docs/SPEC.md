...
- **Gesucht** wird ExifTool über `PATH`. Der Pfad ist überschreibbar. Vorrang: Umgebungsvariable `FOTOSORT_EXIFTOOL`, dann der Konfigurationswert `exiftool_pfad` (Gruppe `[leistung]`, §9), dann das im Windows-Paket mitgelieferte ExifTool (`<Paket>\exiftool\exiftool_files\exiftool.pl`, gestartet über `perl.exe` daneben), dann `PATH`. **Start über perl.exe:** Die Windows-Fassung von exiftool.org besteht aus dem Starter `exiftool(-k).exe`/`exiftool.exe` und dem Ordner `exiftool_files` mit `perl.exe`, `perl532.dll` und `exiftool.pl`; der Starter lädt nur `perl532.dll` und ruft darin Perl mit `exiftool.pl` auf, ohne eigene Umgebungsvariablen. Deshalb startet fotosort, wo immer dieser Aufbau vorliegt (auch bei `FOTOSORT_EXIFTOOL`/`exiftool_pfad` auf ein solches `exiftool.exe`), `perl.exe exiftool.pl` direkt – ein Programm weniger, das ein Virenscanner bei jedem Start prüfen muss (`metadaten.exiftool_befehl`). Lange Pfade behandelt ExifTool dabei selbst (Option `WindowsLongPath`, über das mitgelieferte `Win32::API`). `fotosort --version` nennt Programmversion und das gefundene ExifTool.
...
3. **Übertragen** – wahlweise
   ...
   - **Verschieben**: jede Datei wird einzeln kopiert, geprüft und erst dann in der Quelle gelöscht.
     ...
     **Gebaut (Phase 5):** Nach dem Kopieren wird die Frischlesung beider Seiten im Hash-Worker gemacht; stimmen beide Hashes mit dem beim Kopieren berechneten überein, geht die Datei durch dieselbe Löschstelle wie beim Aufräumen (endgültig; einen Ordner `_geloescht_` gibt es im Verschieben-Modus nicht, denn Verschieben soll die Quelle freigeben). Weicht die Quelle ab: `analysiert`, Hash geleert, Ereignis `quelle_seit_kopieren_geaendert`, die schon geschriebene Kopie bleibt im Ziel. Weicht das Ziel ab: `fehler`, Ereignis `loeschung_verweigert`. Duplikate (gleicher Inhalt schon im Ziel) bleiben in der Quelle, bis `pruefen` sie bestätigt und `aufraeumen` sie entfernt. Bricht das Programm beim Umbenennen zwischen dem Umbenennen und dem Festschreiben von `verschoben` ab, erkennt der nächste Start das an der Spalte `umbenannt` (§6): Quelle weg, Datei unter dem beanspruchten Namen mit passender Größe da → Status `verschoben` nachgetragen (Ereignis `nachtraeglich_bestaetigt`), den Hash liefert `pruefen`.
...
5. **Quelle aufräumen** (nur im Kopier-Modus, nur auf ausdrücklichen Befehl) – gelöscht werden ausschließlich Quelldateien mit Status `geprueft` oder `duplikat_bestaetigt` (§5). Kein anderer Status berechtigt zum Löschen.
   ...
   **Nachgezogen nach der Prüfung von Phase 5 (zwei unabhängige Prüfer, einer als Angreifer):**
   - **Quelle und Ziel dürfen nie dieselbe Datei sein** (§4 Phase 1). Vor jeder Löschung werden Geräte- und Inode-Nummer sowie der aufgelöste Pfad beider Seiten verglichen; bei Gleichheit (Hardlink, Bind-Mount, dieselbe Freigabe über zwei Pfade) wird verweigert, Status `fehler`. Auch der Scan erkennt einen Unterordner, der über einen zweiten Pfad das Ziel ist, an Geräte- und Inode-Nummer und überspringt ihn.
   - **Unmittelbar vor dem Entfernen** wird die Quelle noch einmal angesehen: Geräte-/Inode-Nummer, Größe, Änderungszeit und Statuszeit müssen genau denen entsprechen, die vor der Frischlesung standen. Beim **endgültigen** Löschen (`--endgueltig` und `kopieren --verschieben`) wird die Quelle dazu noch einmal **vollständig gelesen** und gegen den gespeicherten Hash gehalten — im Hauptstrang, direkt vor dem `unlink`. Jede Abweichung heißt „Quelle seit dem Kopieren geändert": nicht löschen, zurück auf `analysiert`. Beim Ordner `_geloescht_` genügt die Kennung, weil dort der aktuelle Inhalt mitwandert und nichts verloren gehen kann. Der Preis: Beim endgültigen Löschen wird die Quelle zweimal gelesen; das ist gewollt (CLAUDE.md, oberste Regel).
   - Eine Frischlesung trägt die **Nummer des Laufs**, in dem sie entstand; eine Lesung aus einem anderen Lauf zählt an der Löschstelle nicht.
   - `bestaetigt_in_lauf` wird bei jedem Neuanfang einer Zeile geleert (zurück auf `analysiert`, neuer Anspruch, `kopiert`, `duplikat`, `fehler`). Eine alte Kennung kann so nie als „unsere Löschung" gelten.
   - Ein **Betriebssystemfehler beim Entfernen** (Schreibschutz, fehlende Rechte, nur lesbare Freigabe) betrifft nur diese eine Datei: Status `fehler` mit Grund, Ereignis `loeschung_verweigert`, der Lauf geht weiter und wird beendet. Im Rückfall des Papierkorbs (Kopieren statt Umbenennen) wird die Kopie vor dem Entfernen der Quelle von der Platte zurückgelesen; scheitert das Entfernen, wird die eigene Kopie wieder zurückgenommen.
...
6. **Leere Ordner entfernen** (eigener, optionaler Schritt) – nur wirklich leere Ordner. Reste wie `Thumbs.db`, `.DS_Store`, `desktop.ini` zählen als leer und dürfen mit entfernt werden (Liste konfigurierbar: `reste_dateien`, §9). Der angegebene Quell-Wurzelordner selbst bleibt stehen.
   ...
   **Gebaut (Phase 5):** `aufraeumen --leere-ordner`, nach Bestätigung mit dem Wort `entfernen`, `--dry-run` zeigt die Ordner. Ein Ordner gilt nur als leer, wenn er ausschließlich leere Unterordner und Reste-Dateien ohne echten Dateityp enthält; Verknüpfungen (Symlinks wie Windows-Junctions, in jeder Tiefe), jede andere Datei und ein im Quellbaum liegendes Ziel machen ihn nicht leer. Ein Rest muss zusätzlich nach seiner Endung `sonstiges` sein: Ein Eintrag wie `beute.jpg` in `reste_dateien` trifft damit nie ein Foto, auch keines, das erst nach dem Scan hinzukam und noch keine Zeile hat. Reste werden erst entfernt, wenn der Ordner sonst leer ist (Ereignis `rest_entfernt`), dann der Ordner (`leerer_ordner_entfernt`). Der Ordner `_geloescht_` bleibt; der Scan betritt ihn nie (in jeder Tiefe, gezählt als ausgeschlossen), und das Aufräumen wählt nie eine Zeile aus, deren Pfad in ihm liegt.
...
- `umbenannt` — `1`, wenn der laufende Anspruch (`kopieren_laeuft`) ein Verschieben durch Umbenennen ist, sonst `0`. Nur damit lässt sich nach einem Absturz ein fertiges Umbenennen (Quelle weg, Zieldatei da) von einer abgebrochenen Kopie unterscheiden (§4 Phase 3).
...
- **Metadaten:** ExifTool nicht pro Datei starten. Mehrere dauerhaft laufende ExifTool-Prozesse (`-stay_open`), jeweils mit Stapeln von Dateien, JSON-Ausgabe, nur die benötigten Felder, `-fast2`. Anzahl Prozesse: Standard nach Profil — `hdd` und `netzwerk` 4, `ssd` Anzahl Kerne, höchstens 16 (`metadaten_prozesse` in §9 überstimmt das). Der erste echte Testlauf (36.000 Dateien auf einer Festplatte, 32 Prozesse = Anzahl Kerne) kam auf 8 Dateien/s: Eine Festplatte liest mit vielen gleichzeitigen Lesern nur noch mit springendem Kopf. Die Prozesse starten gestaffelt (etwa 200 ms Abstand), nicht alle im selben Augenblick. Die Dateien gehen in der Reihenfolge der Quellordner an ExifTool (Stapel gleichen Typs zu 200 Dateien, je Ordner sortiert), damit die Platte möglichst sequentiell liest.
- **Hilfsprozesse unsichtbar:** Unter Windows öffnet weder ein ExifTool-Prozess noch der Arbeitsprozess der Oberfläche ein Konsolenfenster (`prozesse.py`, gilt auch im gepackten Programm): ExifTool und die ExifTool-Prüfung bekommen `CREATE_NO_WINDOW` mit Startinformation `SW_HIDE`; der Arbeitsprozess `DETACHED_PROCESS | CREATE_NEW_PROCESS_GROUP` mit `SW_HIDE` (keine Konsole erben, keine neue öffnen, kein Strg+C aus dem Fenster). Im ersten echten Testlauf ging je ExifTool-Prozess ein schwarzes Fenster auf, und der Virenscanner schlug an. Die Paketprüfung in der CI beobachtet während des Durchlaufs alle Fenster (`EnumWindows`, alle 50 ms): Kein neues Konsolenfenster darf aufgehen. Vorbehalt: Zwei Gegenproben (ein Kind mit `CREATE_NEW_CONSOLE`, eines mit `CREATE_NO_WINDOW`) zeigen vorher, was die Umgebung überhaupt beobachtbar macht; auf einem Läufer ohne Bildschirm kann die Wache nur so viel beweisen, wie die Gegenproben hergeben, und sagt das in ihrer Ausgabe. Die Flaggen selbst prüft `tests/test_prozesse.py` unter Windows direkt.
- **Zeitlimit je ExifTool-Stapel:** Antwortet ExifTool nicht innerhalb von 60 s plus 1 s je Datei des Stapels, wird der Prozess beendet und beim nächsten Stapel neu gestartet; der betroffene Stapel wird Datei für Datei nachgelesen, sodass nur die eine hängende Datei den Status `fehler` („ExifTool hat innerhalb des Zeitlimits nicht geantwortet“) bekommt. Die Analyse nennt am Ende, wie oft das nötig war.
...
fotosort analyse            --ziel <Ziel> [--profil hdd|ssd|netzwerk] [--prozesse N]
...
**Gebaut (Phase 6): `fotosort start`.** Der geführte Modus ist die einzige Ausnahme von „jeder Befehl braucht `--ziel`": Fehlt das Ziel, wird danach gefragt (so lässt er sich unter Windows per Doppelklick starten, Phase 6 `start.bat`); einen nicht vorhandenen Zielordner legt er nur nach Rückfrage an. Danach fragt er nach den Quellordnern (bekannte Quellen des Archivs werden gezeigt, weitere lassen sich ergänzen; ein Ordner, der das Ziel ist oder darin liegt, wird abgelehnt und erneut gefragt), nach Kopieren oder Verschieben und nach dem Profil (`hdd`, `ssd`, `netzwerk`); `--quelle`, `--verschieben` und `--profil` ersetzen die jeweilige Frage. Es folgt eine Zusammenfassung mit der aktuellen Phase (§8 `status`) und ein OK. Dann laufen die Schritte Scan → Analyse → Kopieren/Verschieben → Prüfen → Aufräumen, jeder über denselben Weg wie der einzelne Befehl (eigener Lauf mit `fotosort start …` als Befehl, Sicherung und Bericht nach jedem Schritt), und vor jedem Schritt wird gefragt; Enter heißt ja, `n` hört auf. Ein Schritt ohne Arbeit wird übersprungen und gemeldet — so macht ein erneuter Aufruf genau dort weiter, wo aufgehört wurde. Nach der Analyse zeigt die Zusammenfassung die Kameramodelle; der Nutzer kann Aliase eingeben (Modell → Ordnername). Sie werden in die `config.toml` unter `[kamera.aliase]` eingetragen (Kommentare bleiben, vorhandene Einträge für dasselbe Modell werden ersetzt, geschrieben wird nur ein als TOML gültiges Ergebnis), die betroffenen analysierten Dateien samt Gruppenmitgliedern fallen auf `gefunden` zurück und werden neu analysiert; schon kopierte Dateien bleiben, wo sie sind. Vor dem Kopieren wird der Probelauf gezeigt. Aufräumen und leere Ordner geschehen nur auf ausdrückliches „ja" und danach mit denselben Bestätigungswörtern wie `aufraeumen` (Standard: Ordner `_geloescht_`). Meldet ein Schritt Fehler, wird gefragt, ob es trotzdem weitergehen soll. Ohne Terminal (Pipe, Skript) stellt `start` keine Fragen und tut nichts; Strg+C und eine zu Ende gelesene Eingabe brechen ab, das Bisherige bleibt gespeichert. ExifTool wird vor der ersten Frage geprüft (§2).

**Gebaut (Phase 6): `fotosort messen --quelle A --ziel B`.** Misst das Lesetempo der Quelle (eine Stichprobe echter Dateien wird vollständig gelesen und gehasht, je Stufe andere Dateien, damit der Zwischenspeicher nicht täuscht) und das Schreibtempo des Ziels (Dateien aus Zufallsbytes mit `fsync` in einen vorübergehenden Ordner `.fotosort_messung_…` im Ziel, der am Ende — auch nach Strg+C oder einem Fehler — wieder entfernt wird; nur selbst angelegte Dateien werden entfernt) mit 1, 2, 4 und 8 gleichzeitigen Vorgängen und je etwa `--mb` MB (Standard 256). Legt weder Archiv noch Lauf an und braucht kein ExifTool. Ergebnis: eine Tabelle MB/s je Worker-Zahl und ein Vorschlag für `[leistung]` (`profil`, `kopier_worker`, `hash_worker`): die kleinste Worker-Zahl, die auf 90 % des besten Werts kommt; liegt Quelle oder Ziel auf einem Netzlaufwerk, wird `netzwerk` vorgeschlagen. Ist die Quelle zu klein (unter 16 MB je Stufe) oder der Platz im Ziel zu knapp, wird der jeweilige Teil ausgelassen und gesagt.
...
**Gebaut (Phase 7): die Oberfläche — `fotosort fenster`.** Unter Windows ist sie ein **richtiges Desktop-Programm** (PySide6/Qt, kein Browser, keine Konsole): ein normales Windows-Fenster mit eigenem Taskleisten-Eintrag und Symbol (eigene AppUserModelID, nicht das Python-Symbol), Mindestgröße 1040 × 700, gestartet per Doppelklick auf `start.bat` – das startet das mitgelieferte, signierte `python\pythonw.exe` (Windows-Paket, siehe unten). Alle Ordner werden über den normalen Windows-Dialog gewählt (`QFileDialog.getExistingDirectory`); nirgends muss ein Pfad getippt werden; Quellen lassen sich beliebig viele hinzufügen und entfernen. Das Aussehen ist die Design-Übergabe (`docs/design/`, Nocturne) als Qt-Stylesheet (`oberflaeche/stil.py`) mit der Inter-Schrift aus dem Paket (TrueType, `static/fonts/`). Jeder Fehler vor oder beim Start erscheint in einem verständlichen Meldungsfenster mit Rat und dem Ort des Protokolls (`oberflaeche/meldungsfenster.py`; der Einstieg `lib\fotosort_start.py` fängt alles und wird als Datei gestartet, damit das Meldungsfenster selbst dann erscheint, wenn sich fotosort nicht laden lässt), nie als „Unhandled exception in script". Mit `--ohne-fenster` läuft stattdessen nur der Server der **Browser-Fassung** (dieselben Seiten in `static/`), und die Adresse (`http://127.0.0.1:<Port>/`, nur vom eigenen Rechner erreichbar) wird genannt — der Weg für den späteren Container auf dem Server (Phase 8); dort wird der Pfad eingetippt. `--selbsttest` öffnet das Fenster und schließt es wieder (0/1); `--durchlauf ZIEL QUELLE [--fotos ORDNER]` fährt den ganzen Ablauf über die Bedienelemente des Fensters, wahlweise mit einem Bild je Ansicht — so prüft die CI das Paket (Qt offscreen), und so entstehen die Bilder in `docs/oberflaeche/`. Das Fenster schreibt beim Erscheinen `oberflaeche/fenster.json` und beendet sich auf eine Datei `oberflaeche/schliessen` (für die Prüfung).
...
**Aussehen (Phase 7, zweiter Stand — gilt für Desktop-Fenster und Browser-Fassung):** Das Fenster folgt der Design-Übergabe in `docs/design/` (`DESIGN.md`, `index.html`, `app.css`, `styles.css` — „Nocturne“: dunkler Grund, eine Akzentfarbe nur als Linie, Schrift Inter, Zahlen mit gleicher Ziffernbreite, Pfade und Statuswörter in Monospace, keine Fließtexte, nur Beschriftungen, Zahlen und Pfade). `styles.css` ist die Quelle für Farben, Abstände und Komponenten und wird nicht verändert; die einzige Abweichung der im Programm ausgelieferten Kopie ist die entfernte Zeile, die Inter aus dem Internet lädt — die Schrift liegt als woff2 im Paket (`static/fonts/`, `fonts.css`), Ersatz ist Segoe UI. Ein Fenster, drei Zeilen: Titelzeile (`fotosort — <Ziel>`, rechts `Lauf N`), Inhalt, Statusleiste (`db lokal · Sicherung <Zeit> · Archiv gesperrt/frei`); Mindestgröße 1040 × 700. Die Hauptansicht zeigt die Pfade (Quellen → Ziel, rechts Profil und Modus), die Phasenleiste (Scan · Analyse · Kopieren · Prüfen · Aufräumen mit Akzentlinie als Fortschritt), links Balken, Kennzahlen (Dateien, Datenmenge, MB/s, Restzeit), den laufenden Vorgang und die Zähler des letzten Schritts, rechts die Karten „Aufräumen · Schritt 5“ (Bestätigungswort; der Knopf wird erst aktiv, wenn das Wort exakt stimmt), „Leere Ordner“ und „Letzter Bericht“ (Öffnen / CSV); unten die Aktionen (primär = nächster sinnvoller Schritt, gesperrt solange etwas läuft). Startseite, Kamera-Tabelle und Listen sind im selben Stil ergänzt.

Ablauf im Fenster: Startseite mit Zielordner, Liste der Quellordner (bekannte Quellen des Archivs werden gezeigt, neue hinzugefügt oder entfernt; geprüft wie im geführten Modus), Kopieren/Verschieben (Standard Kopieren), Profil als Auswahlliste mit Erklärung, großer Knopf „Los geht's“ (prüft ExifTool nach §2 und startet den Scan; ein fehlender Zielordner wird nur nach Rückfrage angelegt). Wird im Ziel ein angefangenes Archiv erkannt, zeigt die Startseite den Stand in einem Satz und bietet „Weitermachen“ beim offenen Schritt an — ohne erneuten Scan. **Zwei Rückfragen auf der Startseite** (seit dem ersten echten Testlauf): Ist der gewählte Quellordner ein ganzes Laufwerk (`C:\`, `\\server\share\`), der eigene Benutzerordner oder der Ordner aller Benutzer, fragt das Programm mit Erklärung, ob wirklich dieser Ordner gemeint ist (meist ist ein Unterordner wie „Bilder“ gemeint); erst „Trotzdem nehmen“ übernimmt ihn. Ist der Zielordner nicht leer und enthält noch kein Archiv (keine Archiv-Kennung), fragt „Los geht's“, ob es dort weitergehen soll — es wird nichts gelöscht oder überschrieben, aber die Fotos kommen zu dem, was schon da liegt. **„Archiv verwerfen“** auf der Karte des angefangenen Archivs entfernt die Merkliste des Programms zu diesem Ziel — den lokalen Archiv-Ordner (Datenbank, `config.toml`, Sperrdatei) und `.fotosortierer` im Ziel (Kennung, Sicherung, Berichte) — **niemals eine kopierte Datei und nie etwas in einer Quelle**; nicht, solange ein Schritt läuft oder ein anderer Lauf das Archiv belegt (die Archivsperre wird direkt genommen, ohne die Datenbank zu öffnen — Verwerfen muss gerade bei einer kaputten oder veralteten Datenbank gehen); nur nach dem getippten Wort `verwerfen`; nicht über eine Verknüpfung (Symlink, Junction) an Stelle eines der beiden Ordner; und nicht, wenn in einem der beiden Ordner eine Foto-, RAW-, Video- oder Sidecar-Datei liegt (Sicherheitsnetz, geprüft vor der ersten Löschung). Die Protokolle der Oberfläche (`oberflaeche/`) bleiben. Danach ist die Startseite leer, und der Zielordner kann als neues Archiv beginnen. Die Rückfragen kommen erst nach den Prüfungen von Quellen und ExifTool, damit eine beantwortete Frage nicht noch einmal gestellt wird. Je Schritt eine Seite mit Fortschrittsbalken, Zahlen, Pause/Fortsetzen/Abbrechen; nach jedem Schritt eine Zusammenfassung in einfachen Worten aus der Datenbank mit „Weiter: <nächster Schritt>“. Nach der Analyse die Kameramodelle als Tabelle mit tippbarem Ordnernamen; geänderte Namen werden wie im geführten Modus in die `config.toml` eingetragen, die betroffenen Dateien neu analysiert. Verschieben verlangt vor dem Start das Wort „verschieben“. Aufräumen hat eine eigene Seite: Anzahl und Größe je Quelle, Wahl `_geloescht_`-Ordner oder endgültig, Bestätigung durch Tippen des Bestätigungsworts in ein Feld (die Prüfung des Worts geschieht im Kern, nicht in der Seite), leere Ordner mit eigenem Wort. Fehler, Duplikate und Dateien ohne Datum als seitenweise geblätterte Tabellen (100 je Seite). Knöpfe „Bericht öffnen“ (schreibt den Bericht nach §10 und öffnet die Textdatei) und „Einstellungen öffnen“ (`config.toml`). Alle Texte deutsch, jede Frage mit einem Satz Erklärung.

**Windows-Paket (seit v0.6, ohne PyInstaller).** Aus dem Test von v0.5: Norton prüfte jede selbst gebaute exe (PyInstaller) bei jedem Start und hielt sie bis zu 60 Sekunden fest; ein Skript über `python.exe` beanstandete es nie. Deshalb startet im Paket **nur noch das offizielle, von der Python Software Foundation signierte Python** und das Perl von ExifTool – keine eigens gebauten Programme:

- `python\` – das „Windows embeddable package" (64 Bit) von python.org, Version 3.12.10 (die letzte 3.12 mit fertigen Windows-Dateien, wie in den Tests), geprüft über seine SHA-256. Unverändert bis auf `python312._pth`: Suchpfad `python312.zip`, `.` und `..\lib`, **ohne** `import site` – das eingebettete Python liest damit weder Umgebungsvariablen noch Pakete des Rechners.
- `lib\` – fotosort, der Einstieg `fotosort_start.py` und die Bibliotheken für Windows 64 Bit, fest mit Version und Prüfsumme (`paket/windows-bibliotheken.txt`: PySide6-Essentials, shiboken6, blake3, rich, tomli-w, tzdata). Qt ist auf QtCore, QtGui und QtWidgets gekürzt (Plugins: `qwindows`, `qoffscreen`, `qmodernwindowsstyle`, dazu die C++-Laufzeit von Microsoft). Nicht im Paket: die Browser-Fassung (fastapi, uvicorn – unter Windows gibt es das Fenster; `fenster --ohne-fenster` meldet das verständlich), pygments/markdown-it (rich braucht sie nur für Extras, die fotosort nicht nutzt), jede exe von pip oder PySide6. Alles ist vorkompiliert („unchecked-hash"), damit der erste Start schnell ist und nichts in den Programmordner geschrieben werden muss.
- `exiftool\` – ExifTool von exiftool.org: `exiftool_files` mit `perl.exe` und `exiftool.pl`, ohne den Starter `exiftool.exe` (§2).
- `start.bat` – `python\pythonw.exe -X utf8 lib\fotosort_start.py fenster` (ohne Konsole); `fotosort.bat` – dasselbe über `python\python.exe` für Befehle mit Ausgabe. Der Arbeitsprozess der Oberfläche läuft über `pythonw.exe` neben dem laufenden Python. `-X utf8` statt `PYTHONUTF8`, weil das eingebettete Python Umgebungsvariablen nicht liest.
- Der Bau (`paket/bauen.py`, geht auf jedem System) bricht ab, wenn eine Prüfsumme nicht stimmt, eine andere exe als `python.exe`, `pythonw.exe` und `perl.exe` im Paket liegt oder eine Datei eine Bibliothek braucht, die weder im Paket noch in Windows liegt (Importtabellen aller exe/dll/pyd, `paket/paketinhalt.py`). Die Paketprüfung in der CI (`paket/pruefen.py`, windows-latest, frischer PC, Ordner mit Leerzeichen und Klammern) prüft zusätzlich die Signaturen, dass das Fenster in 20 Sekunden erscheint und aus `python\pythonw.exe` des Pakets stammt, dass die Arbeitsschritte über `pythonw.exe` liefen, dass ExifTool über `perl.exe` startet, einen kompletten Durchlauf über das Fenster und die Befehle über `fotosort.bat`.
- Schlägt ein Virenscanner trotzdem an, beschreibt die LIESMICH die Ordner-Ausnahme für Microsoft Defender, Norton und Avast.
...
- **Restzeit (seit v0.6, alle Phasen, Fenster und Konsole):** In den ersten 60 Sekunden und solange weniger als 3 % geschafft sind nur „wird berechnet“ – der Anfang ist oft schneller als der Rest (Zwischenspeicher von Windows). Danach aus einem gleitenden Durchschnitt der letzten 60 Sekunden (erledigte Menge im Fenster geteilt durch seine Dauer; Menge = Datenmenge, sonst Zahl der Dateien), neu berechnet höchstens alle 5 Sekunden, abgerundet auf ganze Minuten, unter 2 Minuten auf 10-Sekunden-Schritte (unter 10 Sekunden: „unter 10 s“). Die Uhr beginnt mit der ersten Fortschrittsmeldung des Schritts; Pausen zählen nicht mit; ging in der letzten Minute nichts voran, bleibt die letzte Zahl stehen. Ohne bekannte Gesamtmenge (Scan) keine Restzeit (`restzeit.py`).
...
- `metadaten_prozesse` — Anzahl dauerhaft laufender ExifTool-Prozesse (§7). `0` bedeutet automatisch nach Profil (`hdd` 4, `netzwerk` 4, `ssd` Anzahl Kerne, höchstens 16). Standard: `0`
